...
Stochastic programming has long served as a fundamental framework for modeling decision-making under uncertainty, primarily through expected-value models and recourse formulations \cite{B55,D55}. As the field matured, methods evolved to address model robustness. Robust optimization \cite{BN98,BS04} proposes a deterministic reformulation considering worst-case outcomes within a defined uncertainty set. A major development is Distributionally Robust Optimization (DRO), which seeks a solution most resilient against the worst-case probability distribution for uncertain parameters, typically within an ambiguity set \cite{DY10}.
DRO has gained significant traction, especially in contexts like logistics and supply chain management \cite{WC20,JWCZ22}, where it provides a powerful structure for prescriptive analytics. Mainstream DRO approaches define the ambiguity set based on measurable moments (e.g., mean and covariance) \cite{Esfahani, Kuhn 2018} or specific distance metrics (e.g., Wasserstein or $\phi$-divergence) between the empirical distribution and candidate distributions \cite{Bertsimas, Kallus 2020}. This methodology provides a strong theoretical guarantee but requires the designer to make a post-hoc choice on the ambiguity set's structure and size, which may not fully reflect the true statistical dynamics of the input data.
A primary technique for applying both stochastic programming and DRO is Scenario-Based Optimization. This approach discretizes the continuous probability distribution of uncertain parameters into a finite set of specific scenarios. The Sample Average Approximation (SAA) method is a pivotal technique here \cite{SH98}, approximating the true underlying distribution with an empirical distribution derived from a sample of independent and identically distributed realizations.
The integration of forecasting and scenario-based optimization—often termed predict-then-optimize—is a growing body of literature \cite{R18,FTW18,BK19}. The typical workflow follows a two-stage paradigm: a forecasting model (e.g., SARIMA, GARCH, LSTM) first estimates future parameters, and a separate scenario generation phase simulates or samples future paths based on the forecast's predictive distribution.
...
This unification of forecasting and scenario generation is a key novelty, offering a compelling alternative to moment-based and distance-based DRO methods in predictive contexts where historical time series are available. The resulting optimization framework, where we seek to minimize expected costs while ensuring constraints are satisfied across the statistically derived scenarios, provides a highly accountable and data-driven prescriptive solution.
