...
%\documentclass[a4paper,fleqn,3p,11pt,authoryear]{cas-sc}
\documentclass{itor}
...
% Short title
%\shorttitle{BESO for two-echelon logistics}    
% Short author
%\shortauthors{L.Fenga, V.Maniezzo}
\title {Bootstrap Enhanced Scenario Optimization, a Case Study in Two-Echelon Logistics}                      

%Authors, affiliations address.
\author[Running Author]{Author Name\affmark{a,$\ast$},  Author Name\affmark{b,c} and Author Name\affmark{a,c}}

\affil{\affmark{a}Department/Oganisation/Institute, Address, City and Postcode, Country}
\affil{\affmark{b}Department/Oganisation/Institute, Address, City and Postcode, Country}
\affil{\affmark{c}Department/Oganisation/Institute, Address, City and Postcode, Country}
\email{1st\_author@domain.com [F. Author]; 2nd\_author@domain.com [S. Author];\\ 3rd\_author@domain.com [T. Author]}

%Author's notes/correspondence etc.
\thanks{\affmark{$\ast$}Author to whom all correspondence should be addressed (e-mail: your@emailaddress.xxx).}

%%Date
\historydate{Received DD MMMM YYYY; received in revised form DD MMMM YYYY; accepted DD MMMM YYYY}


%\author[1]{Livio Fenga}[orcid=0000-0002-8185-2680]
...
%                country={UK}}
%
%\author[2]{Vittorio Maniezzo}[orcid=0000-0002-1220-1235]
%\cormark[2]
...
Under standard bootstrap consistency assumptions, if $m=m(n)\to\infty$ with $m(n)/n\to 0$, then $\mathcal{U}_{m(n)}\Rightarrow P^\star$ and $\Psi{m(n)}(x)\to\mathbb{E}_{P^\star}[c(x,\xi)]$ uniformly on compact $X$, implying $x^\star(m(n))\to x^\star$. For finite $n$,
\begin{equation}
...
All reported tests were run on the same machine as in subsection \ref{Subsec:predictive}, but in this case the implementation language was C++ to allow finer control over the MILP solver used, which was CPLEX v. 22.11.
