...
\RUNTITLE{BESO for two-echelon logistics}
...
\ARTICLEAUTHORS{%
\AUTHOR{Livio Fenga}
\AFF{Center for Simulation, Analytics and Modelling, University of Exeter, \EMAIL{L.Fenga@exeter.ac.uk}} %, \URL{}}
...
\ABSTRACT{%
We propose Bootstrap Enhanced Scenario Optimization (BESO), a data‑driven pipeline that couples Maximum Entropy Bootstrap (MEB)–based resampling with scenario‑based optimization to produce robust tactical prescriptions under short, nonstationary demand series. BESO (i) generates bootstrapped demand scenarios that preserve empirical autocorrelation and nonstationarity via MEB, (ii) optionally biases resampling toward decision‑relevant outcomes through an objective‑augmented term, and (iii) embeds scenarios into a deterministic equivalent allocation model to derive inventory and allocation decisions. We validate BESO on a real two‑echelon retail logistics case (52 stores, four DCs) and on an extensive synthetic benchmark suite. Compared to parametric ETS and common DRO baselines, BESO yields forecasts with lower downstream cost volatility, fewer infeasibilities on held‑out data, and more reliable DC sizing recommendations — especially when series are short or nonstationary. We also document how boostset size, scenario calibration, and splittable‑client policies affect solution cost and computational tractability, producing actionable guidance for practitioners on scenario construction and capacity leasing decisions. BESO thus offers managers a practical, reproducible method to convert limited, volatile demand data into operationally robust allocation and inventory decisions.
}%
...
Stochastic programming has long served as a fundamental framework for modeling decision-making under uncertainty, primarily through expected-value models and recourse formulations \citep{D55,B55}. As the field matured, methods evolved to address model robustness. Robust optimization \citep{BN98,BS04} proposes a deterministic reformulation considering worst-case outcomes within a defined uncertainty set. A major development is {\it Distributionally Robust Optimization} (DRO), which seeks a solution most resilient against the worst-case probability distribution for uncertain parameters, typically within an {\it ambiguity set} \citep{DY10}.
...
According to the classification proposed in \citet{DEKOK2018} our contribution is described by the following typology string: 2,D,D,G|F,C|D,G|b,F,N|SC|E,O
...
\begin{equation}
	\widehat{P}n=\frac{1}{n}\sum{i=1}^n \delta_{\xi_i},
\end{equation}
decision space $X\subset\mathbb{R}^d$, and cost function $c:X\times\Xi\to\mathbb{R}$.  
...
The associated DRO objective is
\begin{equation}
	\Psi_m(x) := \sup_{P\in\mathcal{U}_m} \mathbb{E}_P[c(x,\xi)],
\end{equation}
...
Optimizing formulation (DE) makes determines the quantities that each server must be able to supply to each client, i.e. the total quantity that each server must handle. 
