...
\subsection{Bootstrap Cardinality and Decision--Ambiguity Feedback}
...
$\widehat{P}_n=\frac{1}{n}\sum_{i=1}^n \delta_{\xi_i}$,
...
\end{equation}
whose diameter, extreme points, and support all depend on $m$.

The associated DRO objective is
\begin{equation}
	\Psi_m(x) := \sup_{P\in\mathcal{U}_m} \mathbb{E}_P[c(x,\xi)],
\end{equation}
and the optimizer is defined by
\begin{equation}
	x^\star(m)\in\arg\min_{x\in X}\Psi_m(x).
\end{equation}
...
Under standard bootstrap consistency assumptions, if $m=m(n)\to\infty$ with $m(n)/n\to 0$, then $\mathcal{U}_{m(n)}\Rightarrow P^\star$ and $\Psi_{m(n)}(x)\to\mathbb{E}_{P^\star}[c(x,\xi)]$ uniformly on compact $X$, implying $x^\star(m(n))\to x^\star$. For finite $n$,
\begin{equation}
