...
In recent years, bootstrap resampling has become a valuable tool for overcoming limitations of conventional forecasting methods \citep{hasni2019spare}. By generating simulated time series from observed data, it is particularly effective for non-stationary, non-linear, and low signal-to-noise settings \citep{kreiss2012bootstrap}. It also helps mitigate issues related to small sample sizes \citep{davison1997bootstrap} and model uncertainty \citep{sarris2020exploiting}.

Bootstrap methods have also proven effective in model selection. \citet{fenga2011bootstrap} proposed a bootstrap-based approach for ARMA order selection, improving accuracy in finite samples, and later extended this framework to high-dimensional sparse autoregression using LASSO \citep{fenga2017lasso}. These results highlight the relevance of bootstrap techniques in modern statistical modeling.

However, standard bootstrap assumes independent and identically distributed (i.i.d.) data, which is inappropriate for time series characterized by dependence, autocorrelation, and structural patterns. This limitation is particularly critical for short series, where resampling can distort temporal structure and underestimate variability. In this context, Maximum Entropy Bootstrap (MEB) provides several advantages:

\begin{itemize}
	\item {\it Preservation of dependence structure}: maintains autocorrelation, trend, and seasonality through rank-based reordering and interpolation.
	\item {\it Suitability for short series}: preserves local features and generates plausible data paths under limited observations.
	\item {\it Model-free approach}: avoids parametric assumptions and tuning choices such as block length or ARMA specification.
	\item {\it Smooth pseudo-series generation}: produces continuous series suitable for inference and forecasting.
	\item {\it Maximum entropy principle}: generates least-biased samples consistent with observed data.
	\item {\it Flexibility}: applicable to both stationary and non-stationary series.
\end{itemize}
